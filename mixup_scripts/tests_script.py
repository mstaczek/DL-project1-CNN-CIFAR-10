...
def load_data(data_augumentation, data_augumentation_param):
    ...
    if data_augumentation is not None and data_augumentation != 'random_mix_up':
...
def train_single_model(model, epochs, lr, batch_size, momentum, l2_regularisation, regularisation_dropout, data_augumentation, data_augumentation_param):
    ...
    model.to(device)
    trainer = TrainModelFramework(model, trainset, testset, device=device, mixup_train = data_augumentation=='random_mix_up', mixup_param=data_augumentation_param)
...
def neptune_start():
    run = neptune.init_run(
        project="mstaczek/deep-learning-project-1-cnn",
...
if __name__ == '__main__':
    ...
    # first argument is model name: resnet18, simple_nn, simple_cnn
